...
| 2 | Settings | JSON full-data **export** (all tables → single `.json` file download). | ❌ Not started |
| 3 | Settings | JSON full-data **import / restore** (parse backup file, wipe DB, bulk-insert all tables). | ❌ Not started |
...
| 8 | Cloud sync | Google Drive sync (OAuth2 PKCE flow, no client secrets). | Optional per spec; JSON backup covers v1 |
